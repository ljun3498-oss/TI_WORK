...
- `empty_driverlib_4` 当前编译已无 warning / error。
- 本次只做告警收敛和一处真实逻辑补强，没有重构工程结构。

### 处理内容

- 删除 `FOC_auto_start.C` 中只被赋值、没有被读取的状态变量：
  - `last_encoder_continuous_pos`
  - `last_time_tick`
- 删除闭环分支中只声明未使用的 `Vmag`。
- 保留 `Vmax` 的设计意图，并在闭环电流PI输出后用于 `vd/vq` 限幅：
  - `vd = clampf_val(vd, -Vmax, Vmax);`
  - `vq = clampf_val(vq, -Vmax, Vmax);`

### 维护注意

- 对电压限幅相关 warning，不建议简单删除 `Vmax`。如果变量代表控制约束，应优先确认是否应参与 `vd/vq` 或电压矢量限幅。
- 编译告警清理时应区分“临时调试变量”和“漏用的控制变量”；后者可能隐藏真实控制逻辑缺口。
